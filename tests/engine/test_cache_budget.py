...
def test_max_bank_row_bytes_is_largest_single_bank():
    from freetoken.engine.cache_budget import max_bank_row_bytes, planned_bank_cache_nbytes

    sources = {
        "gate_up_packed": [torch.zeros(1, 4096, 2048, dtype=torch.uint8)],  # row 8 MiB
        "down_packed": [torch.zeros(1, 4096, 1024, dtype=torch.uint8)],     # row 4 MiB
    }
    assert max_bank_row_bytes(sources) == 4096 * 2048
    planned = planned_bank_cache_nbytes(1307, sources)
    assert planned["gate_up_packed"] == 1307 * 4096 * 2048
    assert abs(planned["gate_up_packed"] / (1 << 30) - 10.21) < 0.01


def test_hip_cap_prevents_dsv4_10gib_contiguous_bank():
    """Reproduce the live R9700 failure in arithmetic: greedy auto ~1300 slots
    wants a 10.21 GiB gate_up_packed tensor; HIP cap 4 GiB keeps it at 512."""
    from freetoken.engine.cache_budget import (
        hip_max_contiguous_bank_bytes,
        plan_cache_budget,
        require_slot_cache_contiguous,
    )

    gate_up_row = 4096 * 2048
    per_expert = 13_369_344
    sources = {
        "gate_up_packed": [torch.zeros(1, 4096, 2048, dtype=torch.uint8)],
        "gate_up_scale": [torch.zeros(1, 4096, 128, dtype=torch.uint8)],
        "down_packed": [torch.zeros(1, 4096, 1024, dtype=torch.uint8)],
        "down_scale": [torch.zeros(1, 4096, 64, dtype=torch.uint8)],
    }
    free = 22 * (1 << 30)
    cap = hip_max_contiguous_bank_bytes(free)
    assert cap == 4 * (1 << 30)
    assert cap < 10.21 * (1 << 30)

    greedy, _, _ = plan_cache_budget(
        budget_bytes=18 * (1 << 30),
        per_expert_bytes=per_expert,
        cache_per_page=1024,
        num_experts=256,
        total_experts=43 * 256,
        prefill_overlap=True,
        kv_reserve_pages=1,
        max_slots=43 * 256,
    )
    assert greedy * gate_up_row > 10 * (1 << 30)
    ...
    size, pages, overlap = plan_cache_budget(
        ...
        kv_reserve_pages=1,
        max_slots=43 * 256,
        max_bank_row_bytes=gate_up_row,
        max_contiguous_bytes=cap,
    )
    assert size * gate_up_row <= cap
    assert size >= 512  # 2 * num_experts, prefill overlap stays on
    assert overlap is True
    assert pages >= 1
    require_slot_cache_contiguous(size, sources, max_contiguous_bytes=cap)
    with pytest.raises(RuntimeError, match="10\\.21 GiB|contiguous"):
        require_slot_cache_contiguous(1307, sources, max_contiguous_bytes=cap)


def test_hip_max_bank_gib_env(monkeypatch):
    from freetoken.engine.cache_budget import hip_max_contiguous_bank_bytes

    monkeypatch.setenv("FREETOKEN_HIP_MOE_MAX_BANK_GIB", "2")
    monkeypatch.delenv("FREETOKEN_HIP_MOE_MAX_BANK_FRAC", raising=False)
    assert hip_max_contiguous_bank_bytes(22 * (1 << 30)) == 2 * (1 << 30)


def test_require_slot_cache_contiguous_noop_without_cap():
    from freetoken.engine.cache_budget import require_slot_cache_contiguous

    sources = {"w": [torch.zeros(4, 8, dtype=torch.uint8)]}
    require_slot_cache_contiguous(10_000, sources, max_contiguous_bytes=None)
